...
#include "read_random_number_file.h"
#endif
...
/* MODIFIED - added section to read in the random number file and then FFT it
   Note that this has not been checked to work in parallel mode. */
#ifdef RANDOM_NUMBER_FILE
  ...
  float_kind *(randoms);
  complex_kind *(crandoms);

  randoms = (float_kind *) calloc(Nmesh*Nmesh*(Nmesh+2), sizeof(float_kind));
  crandoms = (complex_kind *) randoms;

  read_random_number_file(Nmesh, randoms);

  float_kind Nmesh_f = (float_kind)Nmesh;
  fftnorm = 1.0/sqrt(Nmesh_f*Nmesh_f*Nmesh_f);

#ifdef SINGLE_PRECISION
    Forward_plan = fftwf_mpi_plan_dft_r2c_3d(Nmesh,Nmesh,Nmesh,randoms,crandoms,MPI_COMM_WORLD,FFTW_ESTIMATE); 
    fftwf_execute(Forward_plan);
    fftwf_destroy_plan(Forward_plan);
#else
    Forward_plan = fftw_mpi_plan_dft_r2c_3d(Nmesh,Nmesh,Nmesh,randoms,crandoms,MPI_COMM_WORLD,FFTW_ESTIMATE);
    fftw_execute(Forward_plan);
    fftw_destroy_plan(Forward_plan);
#endif

  // Resulting crandoms are not normalised. Need to mulitply by 1/Nmesh**3.
    ...
    if((i >= Local_x_start && i < (Local_x_start + Local_nx)) ||
      ...
      for(j = 0; j < Nmesh; j++) {
        ...
        for(k = 0; k < Nmesh / 2; k++) {
          ...
          // p_of_k *= -log(ampl); // why is this not -2*log(ampl) as given by Box-Muller??
...
#ifndef RANDOM_NUMBER_FILE
                cdisp[axes][coord][0] = -kvec[axes] / kmag2 * sqrt(-log(ampl)) * delta * sin(phase);
                cdisp[axes][coord][1] =  kvec[axes] / kmag2 * sqrt(-log(ampl)) * delta * cos(phase);
#else
                cdisp[axes][coord][0] = -kvec[axes] / kmag2 * crandoms[coord][1] * delta * fftnorm;
                cdisp[axes][coord][1] =  kvec[axes] / kmag2 * crandoms[coord][0] * delta * fftnorm;
#endif
              }
            }
              ...
              if(j >= Nmesh / 2) {
                ...
                if(i >= Local_x_start && i < (Local_x_start + Local_nx)) {
                  ...
                  for(axes = 0; axes < 3; axes++) {
                    ...
                    cdisp[axes][coord][1] = -kvec[axes] / kmag2 * sqrt(-log(ampl)) * delta * cos(phase);
#else
                    cdisp[axes][coord][0] = kvec[axes] / kmag2 * crandoms[coord][1] * delta * fftnorm; // Removed overall minus to match MUSIC
                    cdisp[axes][coord][1] = -kvec[axes] / kmag2 * crandoms[coord][0] * delta * fftnorm;
#endif
                  }
                }
